...
def evaluate_decoupling(f, loader):
    accuracy = 0.0
    for i, (X, y) in enumerate(tqdm(loader, desc='Evaluation')):
        preds = jnp.argmax(jax.vmap(f)(X), -1)
        accuracy = (jnp.mean(preds == y) - accuracy) / (i+1)
    print(f'Accuracy = {accuracy*100:.2f}%')
...
def main():
    ...
    out = batch_predict(params, jax.random.normal(key, (2, 784)))
    ...
    for epoch in range(num_epochs):
        ...
        for batch in bar: params, state = step(params, state, *batch)
        evaluate(params, testloader)

    X = batch[0]; N = X.shape[0]

    X, Y, J = collect_information(lambda x: forward(params, x), N, key, n_inputs=784, X=X)
    decoupling = algorithm.cmtf_psd(X, Y, J, rank=13, niters=5, key=key)

    error = function_error(lambda x: forward(params, x), decoupling, X)
    print(f'Decoupling error: {error}')

    evaluate(params, testloader)
    evaluate_decoupling(decoupling, testloader)
